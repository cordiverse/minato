...
interface Login extends GuildSync {
  id: string
  platform: string
  name?: string
  guilds?: Guild[]
...
interface Guild extends GuildSync {
  id: string
  platform2: string
  name?: string
  logins?: Login[]
...
namespace RelationTests {
  ...
  export function select(database: Database<Tables>, options: RelationOptions = {}) {
    ...
    it('manyToMany', async () => {
      ...
      console.dir((await database.select('post', {}, { tags: true }).execute()), { depth: 10})
    })
  }
  ...
  //   it('upsert manyToOne', async () => {
  //     const users = await setup(database, 'user', [])
  //     await setup(database, 'post', [])

  //     users.push({ id: 1, value: 2 })

  //     await database.upsert('post', [{
  //       id: 1,
  //       content: 'post2',
  //       author: {
  //         $create: {
  ...
  //           ...post,
  //           tags: post2TagTable.filter(p2t => p2t.post?.id === post.id).map(p2t => tagTable.find(tag => tag.id === p2t.tag?.id)).filter(x => !!x),
  //         })),
  //       })
  //     }
  ...
  //   it('upsert manyToMany', async () => {
  //     await setup(database, 'user', [])
  //     await setup(database, 'post', [])
  //     await setup(database, 'tag', [])

  //     await database.upsert('user', userTable.map(user => ({
  //       ...userTable.find(u => u.id === user.id)!,
  //       posts: postTable.filter(post => post.author?.id === user.id).map(post => ({
  //         ...post,
  //         tags: post2TagTable.filter(p2t => p2t.post?.id === post.id).map(p2t => tagTable.find(tag => tag.id === p2t.tag?.id)).filter(x => !!x),
